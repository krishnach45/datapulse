...
class CosmosDBClient:
        ...
        try:
            ...
            self._container = await asyncio.to_thread(
                lambda: self._db.create_container_if_not_exists(
                    id=COSMOS_CONTAINER,
                    partition_key={"paths": ["/kpi_type"], "kind": "Hash"}
                )
            )
    ...
    def _seed_mock_data(self):
        ...
        # Inventory alerts
        for pid, name in [("SKU-007", "Laptop Stand"), ("SKU-004", "Yoga Mat"), ("SKU-008", "Water Bottle")]:
    ...
    async def query(self, kpi_type: str, extra_filter: str = "", limit: int = 100) -> List[Dict[str, Any]]:
        """Query Cosmos DB (or mock) by kpi_type."""
        if self._is_mock:
            results = [
                v for v in self._mock_store.values()
                if v.get("kpi_type") == kpi_type
            ]
    ...
    async def upsert(self, document: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await asyncio.to_thread(
            lambda: self._container.upsert_item(document)
        )
