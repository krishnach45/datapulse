...
def make_order(event_id="EVT-001", order_id="ORD-001", customer_id="CUST-1",
               region="Europe", total_amount=50.0, discount_pct=0.0,
               status="completed", payment_method="paypal", kafka_offset=1):
    return dict(event_id=event_id, order_id=order_id, customer_id=customer_id,
                region=region, total_amount=total_amount, discount_pct=discount_pct,
                status=status, payment_method=payment_method, kafka_offset=kafka_offset)

def make_click(event_id="EVT-001", session_id="SESS-001", customer_id="CUST-1",
               product_id="SKU-001", added_to_cart=True, time_on_page_sec=120):
    return dict(event_id=event_id, session_id=session_id, customer_id=customer_id,
                product_id=product_id, added_to_cart=added_to_cart,
                time_on_page_sec=time_on_page_sec)

def filter_valid_orders(orders):
    return [o for o in orders if o["event_id"] and o["order_id"]
            and o["customer_id"] and o["total_amount"] > 0 and o["status"]]
...
class TestOrderCleaning:
    ...
    def test_deduplication_keeps_latest_offset(self):
        orders = [make_order(event_id="EVT-DUP", kafka_offset=1),
...
class TestClickCleaning:
    def test_null_session_filtered(self):
        clicks = [make_click(event_id=None, session_id=None),
    ...
    def test_valid_clicks_pass_through(self):
        assert len(filter_valid_clicks([make_click(),
               make_click(event_id="EVT-002", session_id="SESS-002")])) == 2

class TestGoldAggregation:
    def test_revenue_sum_completed_only(self):
        orders = [make_order(total_amount=100.0, status="completed"),
...
class TestEdgeCases:
    ...
    def test_all_failed_orders_zero_revenue(self):
        orders = [make_order(status="failed"),
                  make_order(event_id="EVT-002", status="pending")]
