...
def create_spark_session() -> SparkSession:
    builder = (
        SparkSession.builder.appName("DataPulse-Gold-Aggregation")
...
def compute_conversion_rate(orders_df: DataFrame, clicks_df: DataFrame) -> DataFrame:
    ...
    added_to_cart = clicks_df.filter(col("added_to_cart")).count()
    ...
    cart_abandonment = (
        round((1 - total_orders / added_to_cart) * 100, 2) if added_to_cart > 0 else 0.0
    )
...
def compute_inventory_alerts(inventory_df: DataFrame) -> DataFrame:
    ...
    return (
        inventory_df.groupBy("product_id", "product_name", "warehouse_id")
        .agg(
...
def write_to_cosmos(df: DataFrame, kpi_type: str):
    ...
    try:
        ...
        for row in rows:
            ...
            if "id" not in doc:
                doc["id"] = (
                    f"{kpi_type}_{doc.get('hour_bucket', doc.get('product_id', 'row'))}_{doc.get('region', '')}"
                )
